<template>
  ...
  <h2>Form 表单</h2>
    ...
    <div class="demo-box">
      <div class="demo">
        <Form :label-width="90" :mode="mode">
          <FormItem :single="true" label="输入框" :required="true"><input type="text" v-model="data.input"/></FormItem>
          <FormItem v-for="(item, index) of data.inputs" :key="item" :label="'输入框'+(index+1)" :required="true">
          ...
          <FormItem label="评分" :required="true">
            <Rate v-model="data.rate" :show-text="true"></Rate>
          </FormItem>
          <FormItem label="多文本" :single="true" :required="true"><textarea rows="3" v-autosize v-model="data.textarea"></textarea></FormItem>
          <FormItem label="单选"><Radio v-model="data.radio" :datas="dataParam"></Radio></FormItem>
          <FormItem label="多选"><Checkbox v-model="data.checkbox" :datas="dataParam"></Checkbox></FormItem>
