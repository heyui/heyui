<template>
  <div class="h-carousel">
    <div class="h-carousel-container" :style="{height: `${height}px`}">
      <div class="h-carousel-list h-carousel-scroll-list" @mouseover="stopAutoplay" @mouseout="startAutoplay" :key="effect"
        v-if="effect=='scroll'">
      ...
      <div class="h-carousel-list" @mouseover="stopAutoplay" @mouseout="startAutoplay" :key="effect" v-else>
        <transition :name="`h-carousel-effect-${effect}`">
          <div class="h-carousel-item h-carousel-effect-item" :key="activeIndex" @click="clickTrigger(activeIndex, carouselItem)">
            <div v-if="!$scopedSlots.item" class="h-carousel-bg" :class="{'h-carousel-bg-pointer': carouselItem.link}"
              :style="{backgroundImage:`url(${carouselItem.image})`}"></div>
    ...
    <ul class="h-carousel-pagination" :class="paginationCls">
      <li class="h-carousel-pagination-item" v-for="(p, index) of datas" :key="index" :class="{'active': isActive(index)}"
        @mouseover="triggerChange('hover', index+1)" @click="triggerChange('click', index+1)">
